@extends('layouts.app')
@section('title', '✏️ تعديل الدرس — تطبيق الخوارزمي')
...
<div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-8 space-y-6" dir="rtl">
    ...
    <!-- Edit lesson form -->
    <form id="lessonEditForm" action="{{ route('admin.lessons.update', $lesson) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')

        <!-- Lesson title -->
        <div>
            <label class="block mb-1 font-semibold text-gray-700 dark:text-gray-300">عنوان الدرس <span class="text-red-500">*</span></label>
            <input name="title"
        ...
        <!-- Lesson content -->
        <div>
            <label class="block mb-1 font-semibold text-gray-700 dark:text-gray-300">نص الدرس</label>
            <textarea name="content" rows="6"
                      class="w-full p-3 rounded-lg border border-gray-300 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-900 dark:text-gray-100 transition">{{ old('content', $lesson->content) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input name="duration_limit" type="number" min="10"
                       value="{{ old('duration_limit', $lesson->duration_limit) }}"
                       class="w-full p-3 rounded-lg border border-gray-300 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-900 dark:text-gray-100 transition">
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', () => {
        ...
        updateBtn.innerHTML = `<svg class="animate-spin w-5 h-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    @if(session('success'))
        Swal.fire({
            ...
            html: `<p class="text-gray-700 text-lg">تم حفظ التعديلات على الدرس بنجاح.</p>`,
            icon: 'success',
            showCancelButton: true,
            confirmButtonText: '📚 الرجوع إلى قائمة الدروس',
            cancelButtonText: '✏️ البقاء في الصفحة',
